...
def Kmeans_attentionMap(image_tensor):
    ...
    kmeans = KMeans(n_clusters=5, mode='euclidean', verbose=1)
...
if __name__ == "__main__":
    ...
    classify_kMeans(labels, smile_image_path_list, image_name)
    k = 5
    closest_images = []
    for i in range(k):
    ...
    for i, img_path in enumerate(closest_images):
        ...
        save_dir = os.path.join(data_file,f"kmeans_{i}")
        save_path = os.path.join(save_dir,f'attentionMap_{image_name}')
