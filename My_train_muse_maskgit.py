...
class Arguments:
    ...
    no_flip: bool = False
    dataset_save_path: Optional[str] = "dataset_64"
    ...
    neutral_column: str = "neutral_image"
    cond_image_column: str = "cond_image"
    base_image_column: str = "base_image"
    log_with: str = "wandb"
    use_8bit_adam: bool = False
    results_dir: str = "image64_maskgit_condimage_multi"
    logging_dir: Optional[str] = None
    vae_path: Optional[str] = r'C:\Style-exprGAN\exprGAN-main\image64_result64_vae\vae.1000010.pt'    
    Encoder_path: Optional[str] = r'C:\Style-exprGAN\exprGAN-main\image64_result64_vae\vae.1000010.pt'    
    ID_Classifier_path: Optional[str] = r'C:\Style-exprGAN\exprGAN-main\image64_result64_vae\vae.1000010.pt'    
    dataset_name: Optional[str] = None
    hf_split_name: Optional[str] = None
    streaming: bool = False
    neutral_data_dir: Optional[str] = r"C:\Style-exprGAN\exprGAN-main\data\neutral_crop_align_128"
    ...
    weight_decay: float = 0.03  #權重衰減是一種正則化技術，用來防止模型過擬合
...
def main():
    ...
    with accelerator.main_process_first():
        if args.no_cache:
            ...
            dataset = get_dataset_from_dataroot(
                args.train_data_dir,
                args.neutral_data_dir,
                base_image_root = args.base_image_dir,
                image_column=args.image_column,
                neutral_column=args.neutral_column,
                cond_image_column=args.cond_image_column,
                base_image_column=args.base_image_column,
    ...
    # model_Encoder = EASNNetwork()
    # EASNNetwork_checkpoint = torch.load(args.Encoder_path)
    # model_Encoder.load_state_dict(EASNNetwork_checkpoint)
    # model_Encoder.requires_grad_(False)
    # model_IdClassifier = IdClassifier()
    # IdClassifier_checkpoint = torch.load(args.ID_Classifier_path)
    # model_IdClassifier.load_state_dict(IdClassifier_checkpoint)
    maskgit = MaskGit(
        vae=vae, 
    ...
    embeds = []
    # bases_maskgit = None
    # if args.bases_maskgit_path is not None:
    #     vae = VQGanVAE(
    #     dim=args.dim,
    #     vq_codebook_dim=64,
    ...
    #     maskgit_checkpoint = torch.load(args.maskgit_path)
