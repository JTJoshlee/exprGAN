...
class ImagecondDataset(ImageDataset):
    def __init__(
        ...
        dataset,
        image_size: int,
        neutral_column = "neutral_image",
        image_column="image",
        cond_image_column="cond_image",
        base_image_column="base_image",
        flip=True,
        center_crop=True,
        stream=False,
        using_taming=False,
        ...
        self.neutral_column = neutral_column
        self.base_image_column = base_image_column
    def __getitem__(self, index):
        smile_image = self.dataset[index][self.image_column]   
        neutral_image = self.dataset[index][self.neutral_column]             
        cond_image = self.dataset[index][self.cond_image_column]
        if self.embeds is not None:
            base_image = self.dataset[index][self.base_image_column]
            return self.transform(smile_image), self.transform(neutral_image), self.transform(cond_image), self.transform(base_image)

        else: 
            return self.transform(smile_image), self.transform(neutral_image), self.transform(cond_image), []
...
def get_dataset_from_dataroot(
    ...
    data_dict = {image_column: [],neutral_column: [], cond_image_column: []}
    for image_path in tqdm(image_paths):
        ...
        neutral_image_target_name = image_name.split("_")[0]
        neutral_image_path = find_image(neutral_image_root, neutral_image_target_name, extensions)
        ...
        if os.path.exists(str(base_image_root)):
            data_dict["base_image_column"] = []
            base_image_path = os.path.join(base_image_root, f"{image_name}.png")
            data_dict[base_image_column].append(str(base_image_path))
            dataset = dataset.cast_column(base_image_column, Image())
        else:
            base_image_path = None
       
        image_path = str(image_path)
        data_dict[image_column].append(image_path)
        data_dict[neutral_column].append(str(neutral_image_path))
        data_dict[cond_image_column].append(str(cond_image_path))
        
    dataset = datasets.Dataset.from_dict(data_dict)
    dataset = dataset.cast_column(image_column, Image())
    dataset = dataset.cast_column(neutral_column, Image())
